        ...
        <div class="w-full max-w-sm">
            <div class="flex flex-col break-words bg-white border-2 shadow-md mt-20">
                ...
                <form class="py-10 px-5" method="POST" action="{{ route('password.email') }}" novalidate>
                    @csrf
                    @if (session('status'))
                    <span class="bg-green-100 border-l-4 border-green-500 text-green-700 p-4 w-full my-5 block text-sm"
                        role="alert">
                    ...
                    <div class="flex flex-wrap">
                        <button type="submit" class="bg-teal-300 hover:bg-teal-700 shadow-md text-white-800
                                 hover:text-white text-sm uppercase
